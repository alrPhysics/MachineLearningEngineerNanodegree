...
class Simulator(object):
    ...
    def __init__(self, env, size=None, update_delay=2.0, display=True, log_metrics=False, optimized=False):
        ...
        if self.log_metrics != False:
            ...
            if a.learning != False:
                if self.optimized: # Whether the user is optimizing the parameters and decay functions
                    ...
                    if self.log_metrics == 'Boltzmann':
                        self.log_filename = os.path.join("../logs", "sim_improved-learning_Boltzmann.csv")
                        self.table_filename = os.path.join("../logs","sim_improved-learning_Boltzmann.txt")
                else:
                    if self.log_metrics:
                    ...
                    if self.log_metrics == 'Boltzmann':
                        self.log_filename = os.path.join("../logs", "sim_default-learning_Boltzmann.csv")
                        self.table_filename = os.path.join("../logs","sim_default-learning_Boltzmann.txt")
        ...
        #Create Log Files for Boltzmann Distribution:
        #if self.log_metrics == 'Boltzmann':
        #    a = self.env.primary_agent
        ...
        #        self.table_file = open(self.table_filename, 'wb')
        #    else:
        #        self.log_filename = os.path.join("logs", "sim_no-learning_Boltzmann.csv") #Doesn't seem necessary, maybe will help avoid bugs.
    ...
    def run(self, tolerance=0.05, n_test=0):
        ...
        while True:
            ...
            if self.log_metrics or self.log_metrics == 'Boltzmann':
    ...
    def render(self, trial, testing=False):
        ...
        if status:
            ...
            if (state['destination'] != state['location'] and state['deadline'] > 0) or (self.env.enforce_deadline is not True and state['destination'] != state['location']):
                ...
                if self.env.primary_agent.learning != False:
